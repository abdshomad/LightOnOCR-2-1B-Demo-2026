...
**Never modify source code inside a git submodule.** The upstream demo lives in [LightOnOCR-2-1B-Demo](LightOnOCR-2-1B-Demo) (see [.gitmodules](.gitmodules)). Treat submodule trees as read-only vendor code.
...
See [docs/design.md](docs/design.md) for rationale and examples (if applicable).
...
## GPU Policy (Mandatory)

**Always use GPU, never fall back to CPU.**
- The server is equipped with 2 NVIDIA L40 GPUs.
- If GPU is not available, identify and stop the GPU-consuming containers/processes (such as other idle OCR servers or VLLM services) to free up VRAM and compute for our testing.
- Never set `CUDA_VISIBLE_DEVICES` to empty or use CPU fallback configurations.
